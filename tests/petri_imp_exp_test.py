...
class PetriImportExportTest(unittest.TestCase):
    def test_importingExportingPetri(self):
        importedPetri1, marking1, fmarking1 = petri_importer.import_net(
        ...
        importedPetri2, marking2, fmarking2 = petri_importer.import_net(
            os.path.join(OUTPUT_DATA_DIR, "running-example.pnml"))

        self.assertEqual(sorted([x.name for x in importedPetri1.places]),
                         sorted([x.name for x in importedPetri2.places]))
        self.assertEqual(sorted([x.name for x in importedPetri1.transitions]),
                         sorted([x.name for x in importedPetri2.transitions]))
        self.assertEqual(sorted([x.source.name + x.target.name for x in importedPetri1.arcs]),
                         sorted([x.source.name + x.target.name for x in importedPetri2.arcs]))
