...
#cd ..
cd python38
docker build --no-cache -t testdockerlinuxpython38 .
docker run testdockerlinuxpython38 bash -c "cd pm4py-source-master/tests && python execute_tests.py"
docker run testdockerlinuxpython38 bash -c "python -c \"import pm4py ; print(pm4py.__version__)\""
