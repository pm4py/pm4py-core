...
def construct_reachability_graph(net, initial_marking):
    '''
        ...
        ----------
        net: Petri net
        initial_marking: initial marking of the Ptri net.
    ...
    re_gr.states.add(ts.TransitionSystem.State(repr(initial_marking)))
    while active:
        curr_mark = active.pop(0)
        curr_state = next((state for state in re_gr.states if state.name == repr(curr_mark)), None)
        en_tr = petri.semantics.enabled_transitions(net, curr_mark)
        for t in en_tr:
            next_mark = petri.semantics.execute(t, net, curr_mark)
            next_state = next((state for state in re_gr.states if state.name == repr(next_mark)), None)
            if next_state is None:
                next_state = ts.TransitionSystem.State(repr(next_mark))
                re_gr.states.add(next_state)
            utils.add_arc_from_to(repr(t), curr_state, next_state, re_gr)
            if hash(next_mark) not in visited and next_mark not in active and hash(curr_mark) != hash(next_mark):
