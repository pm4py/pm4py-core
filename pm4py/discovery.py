...
def discover_performance_directly_follows_graph(log: Union[EventLog, pd.DataFrame], aggregation="mean"):
    """
    Discovers a performance directly-follows graph from a log object
    ...
    log
        Log object
    aggregation
        Type of aggregation to apply: mean, median, min, max, stdev, sum
        Default: mean
    ...
    if check_is_pandas_dataframe(log):
        ...
        from pm4py.util import constants
        from pm4py.algo.discovery.dfg.adapters.pandas.df_statistics import get_dfg_graph
        performance_dfg = get_dfg_graph(log, activity_key=properties[constants.PARAMETER_CONSTANT_ACTIVITY_KEY],
                            timestamp_key=properties[constants.PARAMETER_CONSTANT_TIMESTAMP_KEY],
                            case_id_glue=properties[constants.PARAMETER_CONSTANT_CASEID_KEY],
                            measure="performance", perf_aggregation_key=aggregation)
    else:
        from pm4py.algo.discovery.dfg import algorithm as dfg_discovery
        properties["aggregationMeasure"] = aggregation
        performance_dfg = dfg_discovery.apply(log, variant=dfg_discovery.Variants.PERFORMANCE, parameters=properties)
