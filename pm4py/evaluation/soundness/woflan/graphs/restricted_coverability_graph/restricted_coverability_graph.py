    ...
    while new_arc:
        ...
        while len(nodes)>0:
            ...
            if not np.inf in coverability_graph.nodes[m]['marking']:
                possible_markings = helper.enabled_markings(firing_dict, req_dict, coverability_graph.nodes[m]['marking'])
                m2=None
    ...
    for node in coverability_graph.nodes:
        if all(np.less_equal(coverability_graph.nodes[node]['marking'], marking[0])):
            if coverability_graph.nodes[node]['marking'][index]<marking[0][index]:
                if nx.has_path(coverability_graph, look_up_indices[np.array2string(coverability_graph.nodes[node]['marking'])], current_node):
                    return True
    return False
