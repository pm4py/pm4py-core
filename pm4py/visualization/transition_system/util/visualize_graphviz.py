...
def visualize(ts, parameters=None):
    ...
    for s in ts.states:
        if show_names:
            if s in fillcolors:
                viz.node(str(hash(s.name)), style="filled", fillcolor=fillcolors[s])
            else:
                viz.node(str(hash(s.name)), str(s.name))
        else:
            if s in fillcolors:
                viz.node(str(hash(s.name)), "", style="filled", fillcolor=fillcolors[s])
            else:
                viz.node(str(hash(s.name)), "")
    # arcs
    for t in ts.transitions:
        if show_labels:
            viz.edge(str(hash(t.from_state.name)), str(hash(t.to_state.name)), label=t.name)
        else:
            viz.edge(str(hash(t.from_state.name)), str(hash(t.to_state.name)))
