...
class IM(InductiveMinerFramework[UCL]):

    def fall_through(self, obj: UCL):
        ...
        return self._recurse(ft[0], ft[1])

    def apply(self, obj: UCL) -> ProcessTree:
        ...
        if tree is None:
            tree = self.fall_through(obj)
        return tree

    def _recurse(self, tree: ProcessTree, logs: List[UCL]):
