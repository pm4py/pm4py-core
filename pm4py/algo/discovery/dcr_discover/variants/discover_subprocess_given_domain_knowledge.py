...
def apply(log, findAdditionalConditions=True, inBetweenRels=False, **kwargs):
    ...
    for i in range(len(basic_dcr['events'])//2-1):  # max number of subprocesses if grouped as two events
        # (does not account for nesting)
        subprocesses2 = get_subprocesses(subprocess_dcr)
        print(f'[Take {i+1}] {subprocesses}')
        subprocesses = merge_subprocesses([subprocesses, subprocesses2])
        # create a projected log based on the subprocess events
        subprocess_log = get_subprocess_log(event_log, subprocesses)
        # now run the dcr graph with the subprocess events replaced as the subprocess
        subprocess_dcr, la = alg.apply(subprocess_log, findAdditionalConditions=findAdditionalConditions)
        ...
        if len(s) > 1:
            ...
            for k, v in sp.items():
                ...
                if len(common_events) > 0:
                    ...
                    elif set(common_events) == set(s):
                        # exactly the same sets
                        pass
                    else:
                        # equal length and not the same
                        share_prefix = True
                        s_copy = list(s.copy())
                        prefix = s_copy[0].split('_')[0]
                        i = 1
                        while i < len(s_copy):
                            share_prefix = share_prefix and (prefix == s_copy[i].split('_')[0])
                            i = i + 1
                        if share_prefix:
                            sp[k] = s
                        pass
