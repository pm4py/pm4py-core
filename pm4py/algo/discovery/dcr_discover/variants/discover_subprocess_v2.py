...
def apply_mutual_exclusion(log, findAdditionalConditions=True, atMostOnce=True, inBetweenRels=False, **kwargs):
    event_log = log
    basic_dcr, la = alg.apply(event_log, alg.DCR_BASIC, findAdditionalConditions=findAdditionalConditions)

    cluster_pair, ee = find_mutual_exclusion_clusters(basic_dcr)
    G = nx.DiGraph()
    ...
    for c in cc:
        sps = frozenset()
        if atMostOnce:
            sps = frozenset(c.intersection(la['atMostOnce']))
        else:
            sps = frozenset(c)
        if len(sps) > 1:
            subprocesses[f'{subprocess_notation}{i}'] = sps
            i = i + 1
    ...
    #         i = i + 1
    # now create the dcr graph inside the subprocess
    sp_dcr_dict = {}
    ...
    subprocess_dcr, la = alg.apply(subprocess_log, alg.DCR_BASIC, findAdditionalConditions=findAdditionalConditions)
    if inBetweenRels:
