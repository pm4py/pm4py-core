...
def handle_state(open_set, curr_cost, curr_G, curr_trace, current, moves, cost_function, execute_fn=None, update_trace=True):
...
def apply_trace(trace, G, parameters=None):
    ...
    internal_log_move_cost_function = parameters.get(Parameters.INTERNAL_LOG_MOVE_COST_FUNCTION.value, {})
    ...
    while open_set:
        ...
        if hasattr(curr_trace, 'empty'):  # DataFrame
            is_empty = curr_trace.empty
            first_activity = curr_trace.iloc[0, 0] if not is_empty else None
        elif isinstance(curr_trace, list):  # List
            is_empty = not bool(curr_trace)
            first_activity = curr_trace[0] if curr_trace else None
        elif isinstance(curr_trace, Trace):  # Trace object
            is_empty = not bool(curr_trace)
            first_activity = curr_trace[0][activity_key] if curr_trace else None
        else:
        ...
        if curr_trace and enabled(curr_G, curr_trace[0]):
            handle_state(open_set, curr_cost, curr_G, curr_trace, current, moves, sync_cost_function,
                         execute_fn=execute, update_trace=True)

        # Model Moves
        if enabled(curr_G, curr_trace[0]):
            handle_state(open_set, curr_cost, curr_G, curr_trace, current, moves, model_move_cost_function,
                         execute_fn=execute, update_trace=False)

        # Log Moves
        if curr_trace:
            handle_state(open_set, curr_cost, curr_G, curr_trace, current, moves, log_move_cost_function,
