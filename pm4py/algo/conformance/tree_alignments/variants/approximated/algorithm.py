...
def __approximate_alignment_on_loop(pt: ProcessTree, trace: Trace, a_sets: Dict[ProcessTree, Set[str]],
    ...
    for i in x_variables:
        for j in x_variables[i]:
            x_variables[i][j] = True if x_variables[i][j].varValue else False
    for i in t_variables:
        for j in t_variables[i]:
            t_variables[i][j] = True if t_variables[i][j].varValue else False

    if not x_variables == x__variables:
        print("\nloop x_variables")
        print(x_variables)
        print(x__variables)
        print(points)
        ...
        input()
    if not t_variables == t__variables:
        print("\nloop t_variables")
        print(t_variables)
        print(t__variables)
        print(points)
...
def __approximate_alignment_on_sequence(pt: ProcessTree, trace: Trace, a_sets: Dict[ProcessTree, Set[str]],
    ...
    for i in x_variables:
        for j in x_variables[i]:
            x_variables[i][j] = True if x_variables[i][j].varValue == 1.0 else False

    if not x_variables == x__variables:
        print("\nsequence x_variables")
        print(x_variables)
        print(x__variables)
        print(points)
        input()
