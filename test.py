...
print((functools.reduce((lambda x, y: x + y), list(map(lambda t: len(t), log)))) / len(log))

start = time.time()
log = log_instance.TraceLog(filter(lambda t: len(t) > 12, log), attributes=log.attributes, classifiers=log.classifiers, omni_present=log.omni_present, extensions=log.extensions)
